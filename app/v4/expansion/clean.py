...
def clean_entity(
    ...
    # ----------------------------------------------------------------------
    # Шаг 6.1: Финальная подчистка от оставшихся @DOT
    # ----------------------------------------------------------------------
    # Некоторые @DOT могли остаться из-за особенностей регулярок или граничных случаев
    # Например: когда точка была в конце строки без пробела после неё
    # или когда инициал был последним в предложении
    if '@DOT' in text:
        if verbose:
            logger.warning(f"         найдены незаменённые @DOT, исправляем")
        text = text.replace('@DOT', '.')
