...
def is_likely_sentence(text: str, original_length: int, config: dict, verbose: bool = False) -> bool:
    """
    Простая проверка, похож ли расширенный текст на предложение.
    
    Признаки:
    1. Слишком много слов (> 8)
    2. Слишком большое соотношение длин (> 5)
    3. Есть знаки препинания внутри (кроме дефисов)
    """
    words = text.split()
    
    # ----------------------------------------------------------------------
    # ПРИЗНАК 1: слишком много слов
    # ----------------------------------------------------------------------
    if len(words) > 8:
        if verbose:
            logger.warning(f"         слишком много слов ({len(words)} > 8)")
        return True
    
    # ----------------------------------------------------------------------
    # ПРИЗНАК 2: слишком большое соотношение длин
    # ----------------------------------------------------------------------
    ratio = len(text) / original_length if original_length > 0 else 0
    if ratio > config.get('max_sentence_ratio', 5.0):
        if verbose:
            logger.warning(f"         слишком большое соотношение длин ({ratio:.1f} > {config.get('max_sentence_ratio', 5.0)})")
        return True
    
    # ----------------------------------------------------------------------
    # ПРИЗНАК 3: знаки препинания внутри (кроме разрешённых)
    # ----------------------------------------------------------------------
    sentence_endings = {'.', '!', '?'}
    for char in sentence_endings:
        if char in text:
            if verbose:
                logger.warning(f"         найден знак конца предложения '{char}'")
            return True
...
def expand_to_full_word(
    ...
    # =========================================================================
    # Проверяем, не превратилось ли наше расширение в целое предложение
    # Если да - возвращаем исходный текст без расширения
    # =========================================================================
    if is_likely_sentence(full_word, len(text), config, verbose):
        if verbose:
            logger.warning(f"      ⚠️ расширение похоже на предложение, отменяем")
