...
class WordExpander:
    ...
    def _expand_to_full_word(self, text: str, start_pos: int, end_pos: int,
        ...
        if left_expanded or right_expanded:
            ...
            # Если там есть разделители, значит слева от слова есть другой токен,
            # и левое расширение нужно отменить
            # ----------------------------------------------------------------------
            if left_expanded:
                if self.config.get('verbose', False):
            ...
            # Если там есть разделители, значит справа от слова есть другой токен,
            # и правое расширение нужно отменить
            # ----------------------------------------------------------------------
            if right_expanded:
                if self.config.get('verbose', False):
            ...
            # =========================================================================
            # ВНИМАНИЕ: Эта проверка выполняется ТОЛЬКО если после проверок 1 и 2
            # у нас все еще есть активные расширения (left_expanded или right_expanded).
            #
            # Она проверяет символы между start_pos и end_pos. Это важно для случаев,
            # когда внутри исходного диапазона есть разделители, но мы не расширялись
            # ни влево, ни вправо (например, 'Алданов… Удивительная').
            #
            # НО! Для случая 'та' мы уже расширились влево, и проверка 1 прошла успешно.
            # Запятая на позиции 1831 находится ПОСЛЕ слова (слово заканчивается на 1832),
            # поэтому она не должна влиять на левое расширение. Однако она попадает в
            # диапазон start_pos-end_pos (1830-1832), и если мы здесь ее обнаружим,
            # то ошибочно отменим всё расширение.
            #
            # Поэтому мы проверяем только те символы, которые находятся ВНУТРИ слова,
            # то есть до конца нашего расширенного слова (word_end).
            # =========================================================================
            if left_expanded or right_expanded:
                # Определяем реальные границы слова после всех расширений
                ...
                # Проверяем только ту часть исходного диапазона, которая находится ВНУТРИ слова
                ...
                for pos in range(check_start, check_end):
                    if pos >= len(original_text):
                        break
                    char = original_text[pos]
                    # Если находим разделитель внутри слова - это проблема (например, многоточие внутри)
                    if char in self.WORD_BREAKS and char not in self.ALL_QUOTES and char != '-':
                        if self.config.get('verbose', False):
                            logger.warning(f"         найден разделитель '{char}' на позиции {pos} внутри слова - расширение отменяется")
                        # Полностью отменяем расширение
                        left_expanded = False
                        right_expanded = False
                        word_start = start_pos
                        word_end = end_pos
